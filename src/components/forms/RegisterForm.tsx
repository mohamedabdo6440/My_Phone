...
const RegisterForm = () => {
    ...
    let [phoneUser , setPhoneUser] : any= useState()
    ...
    const submitHandler: React.FormEventHandler = async (e) => {
        ...
        // const payload = { ...rest, phoneNumber: values.phoneCode + values.phone }

        let validateResponse =  validateForm()
      

        if(validateResponse.error){
            toast.error(validateResponse.error.details[0].message)
        }else if(phone(phoneUser).isValid == false){
            toast.error('kindly enter a valid phone number')
        }else{
            dispatch(setLoading(true))
            try {
                const result = await axios.post('customers/first_creation', {
                    'email': values.email,
                ...
                if (result.status === 201) {
                    dispatch(openModal(values.email))
                  ...
                  for(let key in result.data) { 
                       toast.error(key + ': ' + result.data[key]); 
                    }
                   dispatch(setLoading(false))
                }
            } catch (error) {
        ...
        <div className={`${styles.container} ${theme === 'light' ? null : styles.dark} form_close`}>
            <form className={styles.form} onSubmit={submitHandler}>
                ...
                <div className={styles.input_phone}>
                    {/* <select name='phoneCode' onChange={(e)=>setPhoneCode(e.target.value)}>
                ...
                <PhoneInput
                 ...
                 name='phone'
                 onChange={(e:any)=>{setPhoneUser(e)
                }
                 }/>
                    {/* <input type='number' placeholder='Phone Number' value={phone} name='phone' onChange={(e)=>setPhone(e.target.value)} /> */}
                </div>
                <input type='date' placeholder='Birth Date' name='birth_date' onChange={handleChange} />
                {/* <select name='gender' id='gender' value={values.gender} onChange={handleChange}>
                    ...
                    <button
                    type='submit'
                    disabled={loading || !values.birth_date || !values.email || !values.first_Name || !values.last_name || !values.password || !phone}
                > 
                    Register
                </button>
                }
