...
const Home: NextPageWithLayout = () => {
    ...
    const handleNavigateToOtherPages = ({page, route}: {page: HomePage; route: string}) => {
    ...
    const Button = (props:{route:any , title:HomePage})=>{
        return (
        <button
        onClick={() => handleNavigateToOtherPages({page: props.title, route: props.route})}
        className={`${page === props.title ? styles.page_active : null} capitalize`}
        >
            {props.title}
        </button>
        )
    ...
    const ButtonFilter = (props:{title:HomePage})=>{
        ...
        className={`${page === props.title ? styles.page_active : null} font-bold capitalize`}
        >
            {props.title}
        </button>
        )
    }
    const Card = (props:{title:string,image:{ light:any, dark:any, alt:string },params:{page:HomePage,route:any}})=>{
        return (
            <div>
                <div className= {`${styles.serviceCardStyle}`} onClick={() => handleNavigateToOtherPages(props.params)}>
                    <Image src={theme === "light" ? props.image.light : props.image.dark} alt={props.image.alt} />
                    <h4>{props.title}</h4>
                </div>
            </div>
        )
    ...
    return (
        <header className={styles.header}>
            ...
            <div>
                <div className={`${styles.pages_mobile} ${theme === "light" ? styles.light : styles.dark}`}>
                ...
                <div className={`${styles.option} ${theme === "light" ?  `${styles.light} boxShadow`: styles.dark}`}>
            ...
            <div className={`${styles.card_wrapper} ${theme === "light" ? styles.light : styles.dark}`} onClick={()=>{
                                    dispatch(start({}))
                                    dispatch(setDataToNull({}))
                                    dispatch(startSell({}))
                                }}>
                <Card title={"Buy Smart"} image={{light: buysmart,dark:buysmartdark,alt:"Buy Smart Icon"}} params={{page:"buy",route:routes.BUY}}  />
                <Card title={"Sell Smart"} image={{light: sellsmart,dark:sellsmartdark,alt:"Sell Smart Icon"}} params={{page:"sell",route:routes.SELL}}  />
                <Card title={"Repair Smart"} image={{light: repairsmart,dark:repairsmartdark,alt:"Repair Smart Icon"}} params={{page:"repair",route:routes.REPAIR}}  />
                <div>
                    <div>
